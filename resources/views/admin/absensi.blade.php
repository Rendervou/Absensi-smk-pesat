<x-app-layout>
    ...
    <section class="bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-800 p-6 sm:p-10 min-h-screen">
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg mb-8 border border-gray-200 dark:border-gray-700">
            <form action="{{ route('admin.presensi.index') }}" method="get" class="flex flex-col lg:flex-row items-stretch lg:items-center gap-5">
                <!-- Filter Kelas -->
                <div class="flex-1">
                ...
                <div class="lg:self-end">
                    <a href="{{ route('admin.presensi.index') }}" class="w-full lg:w-auto min-w-[140px] rounded-xl bg-gray-500 hover:bg-gray-600 text-white font-semibold text-sm px-6 py-3 shadow-md transition-all duration-300 hover:shadow-lg flex items-center justify-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @else
        <form id="formAbsensi" action="{{ route('admin.presensi.store') }}" method="POST" class="space-y-8">
            @csrf
